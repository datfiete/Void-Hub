...
local Window = Rayfield:CreateWindow({
   Name = "Anime Baddie RNG",
   Icon = 0, -- Icon in Topbar. Can use Lucide Icons (string) or Roblox Image (number). 0 to use no icon (default).
   LoadingTitle = "Anime Baddie RNG script",
   LoadingSubtitle = "by Datfeite",
   ShowText = "Anime Baddie RNG", -- for mobile users to unhide Rayfield, change if you'd like
...
local AutoNextZone = false
local AutoEquipBest = false

local Toggle = MainTab:CreateToggle({
   Name = "Auto Spin",
   CurrentValue = false,
   Flag = "AutoSpinToggle1", -- A flag is the identifier for the configuration file; make sure every element has a different flag if you're using configuration saving to ensure no overlaps
   Callback = function(Autsptgl)
    AutoSpin = Autsptgl 
    print("Auto Spin Active")
   end,
})
...
local Toggle = MainTab:CreateToggle({
   Name = "Auto Next Zone",
   CurrentValue = false,
   Flag = "AutoNextZoneToggle1", -- A flag is the identifier for the configuration file; make sure every element has a different flag if you're using configuration saving to ensure no overlaps
   Callback = function(Auneztgle)
    AutoNextZone = Auneztgle 
    print("Auto Next Zone Active")
   end,
})

task.spawn(function()
    while true do 
        if AutoNextZone then 
            local args = {
	        "requestPurchaseZone"
            }
...
local Toggle = MainTab:CreateToggle({
   ...
   Callback = function(AutbgleBesw)
    AutoEquipBest = AutbgleBesw 
    print("Activated Auto Equip best")
   end,
})
