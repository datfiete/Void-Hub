...
export type ESPWorldRendererHandle = {
    Gui: ScreenGui,
    SetLayout: (self: ESPWorldRendererHandle, layout: { [string]: any }) -> (),
    SetEnabled: (self: ESPWorldRendererHandle, enabled: boolean) -> (),
    IsEnabled: (self: ESPWorldRendererHandle) -> boolean,
    SetMaxDistance: (self: ESPWorldRendererHandle, distance: number) -> (),
    Clear: (self: ESPWorldRendererHandle) -> (),
    Destroy: (self: ESPWorldRendererHandle) -> (),
}
...
local function colorFrom(value: any, fallback: Color3): Color3
...
local function replaceTokens(text: string, sample: { [string]: any }): string
    ...
    for token, replacement in replacements do
        text = string.gsub(text, token, replacement)
    end
    return text
end

local function getWeapon(character: Model): string
...
local function getCharacterBounds(character: Model): (CFrame, Vector3)?
...
local function projectBounds(camera: Camera, cf: CFrame, size: Vector3): (number, number, number, number, number)?
    local half = size * 0.5
    local points = {
...
local function rayVisible(camera: Camera, origin: Vector3, target: Vector3, character: Model): boolean
...
function ESPWorldRenderer.new(options: RendererOptions?): ESPWorldRendererHandle
...
function ESPWorldRenderer:_ensurePlayer(player: Player)
...
function ESPWorldRenderer:_clearPlayerGui(player: Player)
...
function ESPWorldRenderer:_removePlayer(player: Player)
    local maid = self._PlayerMaids[player]
    if maid then
        maid:DoCleaning()
...
function ESPWorldRenderer:_makeVisuals(player: Player): Frame
...
function ESPWorldRenderer:_destroyChildren(root: Frame)
    for _, child in root:GetChildren() do
        child:Destroy()
    end
end

function ESPWorldRenderer:_renderElement(root: Frame, element: { [string]: any }, box: { x: number, y: number, w: number, h: number }, screenWidth: number, screenHeight: number, sample: { [string]: any })
    ...
    label.TextXAlignment = if element.Anchor == "Left" then Enum.TextXAlignment.Left elseif element.Anchor == "Right" then Enum.TextXAlignment.Right else Enum.TextXAlignment.Center
...
function ESPWorldRenderer:_render()
    if not self._Enabled then
        if self._Visuals then
            for _, root in self._Visuals do
                root.Visible = false
            end
        end
        return
    ...
    local boxDesign = nil
    for _, element in layoutElements do
    ...
    for _, player in Players:GetPlayers() do
        if player == localPlayer and self._IgnoreLocalPlayer then
            continue
        end
        self:_ensurePlayer(player)
        ...
        if self._TeamCheck and localPlayer.Team ~= nil and player.Team == localPlayer.Team then
            continue
        end
        if self._VisibleCheck and not rayVisible(camera, camera.CFrame.Position, rootPart.Position, character) then
            continue
        end
        ...
        local projected = projectBounds(camera, cf, size)
        if not projected then
            continue
        end

        local minX, minY, maxX, maxY = projected
        ...
        local teamName = if player.Team then player.Team.Name else "Neutral"
        local health = humanoid.Health
        local maxHealth = humanoid.MaxHealth
        local percent = health / math.max(maxHealth, 1)
        local state = if percent <= 0.25 then "Low Health" elseif percent < 1 then "Injured" else "Normal"
        local sample = {
...
function ESPWorldRenderer:SetLayout(layout: { [string]: any })
...
function ESPWorldRenderer:SetEnabled(enabled: boolean)
...
function ESPWorldRenderer:IsEnabled(): boolean
    return self._Enabled
end

function ESPWorldRenderer:SetMaxDistance(distance: number)
    self._MaxDistance = math.max(tonumber(distance) or self._MaxDistance, 0)
end

function ESPWorldRenderer:Clear()
    if self._Visuals then
        for player, root in self._Visuals do
            root:Destroy()
            self._Visuals[player] = nil
        end
    end
...
function ESPWorldRenderer:Destroy()
    self:Clear()
    for player, maid in self._PlayerMaids do
        maid:DoCleaning()
        self._PlayerMaids[player] = nil
    end
